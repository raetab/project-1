...
#include <ctype.h>
    ...
    char *code;// this is the key in the substitution codes (case 3 & case 4)
    char *char_to_find; //a strings whos value we want to find
...
int main(){   
   ...
   //using switch case statements to implement the above 4 options 
    ...
    case 1:
     ...
     /* the following is a rotation decryption code; this will allow the user to type in a text and dectrypt it using a desiered key */ 
     case 2:
	...
	// below decrypts upper case letters; leaving numbers, punctuation and grammar the same as in the message
     ...
     /* the following is a substitutiom encryption code; this will allow the user to type in a text and enctrypt it using a desiered key */ 
     case 3:
         /* the initial for loop allows for the incementation of the key code*/  
            for(int i = 0 ; i < 26; i ++){
          ...
          /*this for loop will go through the message character by character and will change the character to that desginated by the key which is 
          *code, this is inputed by the user. The toupper function is used to change letters from lower case to upper case; the - A is used to get
          the encryption index, by zeroing the first value of the inputted key*/
     ...
     /* the following is a substitutiom denryption code; this will allow the user to type in a text and dectrypt it using a desiered key */ 
     case 4:
        /* the initial for loop allows for the incementation of the key code*/  
            for(int i = 0 ; i < 26; i ++){
                if(code[i] == char_to_find){
            ...
            for(int i = 0; i < length; i++){
                int decryption_index = toupper(message[i]) - 'A';
                    if(decryption_index >= 0 && decryption_index < 26){
                       find_index = (toupper(message[i]));// find index will allow the charcter to be found in the message
